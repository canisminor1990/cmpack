  ...
  if ((0, _isPlainObject2.default)(entry)) {
    ...
    return Object.keys(entry).reduce(function(memo, key) {
      return !Array.isArray(entry[key])
        ...
        : _extends({}, memo, _defineProperty({}, key, entry[key]));
    }, {});
  }
...
function getEntry(filePath, isBuild) {
  ...
  var value = isBuild
    ? [filePath]
    : [require.resolve('react-dev-utils/webpackHotDevClient'), filePath];
...
function getFiles(entry, cwd) {
  if (Array.isArray(entry)) {
    ...
    (0, _assert2.default)(
      typeof entry === 'string',
      'getEntry/getFiles: entry type should be string, but got ' +
        (typeof entry === 'undefined' ? 'undefined' : _typeof(entry))
    );
