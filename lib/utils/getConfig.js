...
function merge(oldObj, newObj) {
  for (var key in newObj) {
    if (Array.isArray(newObj[key]) && Array.isArray(oldObj[key])) {
      ...
      (0, _isPlainObject2.default)(newObj[key]) &&
      (0, _isPlainObject2.default)(oldObj[key])
    ) {
...
function getConfig(configFile, paths) {
  ...
  if ((0, _fs.existsSync)(rcConfig)) {
    if (process.env.NODE_ENV === 'development' && (0, _fs.existsSync)(jsConfig)) {
      _cmlog2.default.warn(
        'Config error: You must delete ' + rcConfig + ' if you want to use ' + jsConfig
      );
    }
    return (0, _parseJsonPretty2.default)(
      (0, _stripJsonComments2.default)((0, _fs.readFileSync)(rcConfig, 'utf-8')),
      configFile
    );
