...
function getResolve(config, paths) {
  return {
    resolve: {
      ...
      extensions: [].concat(_toConsumableArray(config.extraResolveExtensions || []), [
        '.web.js',
        '.web.jsx',
        '.web.ts',
        '.web.tsx',
...
function getCSSRules(env, _ref3) {
  ...
  function isExclude(modulePath) {
    if (config.cssModulesExclude && config.cssModulesExclude.length) {
      return config.cssModulesExclude.some(function(item) {
        return (
          (0, _winPath2.default)((0, _path.join)(paths.appDirectory, item)).indexOf(
            (0, _winPath2.default)(modulePath)
          ) > -1
        );
      });
    }
  ...
  if (env === 'production')
    rules.forEach(function(rule) {
      return rule.use.unshift(_miniCssExtractPlugin2.default.loader);
    });
  return rules;
}
