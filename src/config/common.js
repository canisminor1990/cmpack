...
export function getCSSRules(env, { config, paths, cssLoaders, theme }) {
  ...
  let rules = [
    ...
    },
    {
      ...
      use: ['style', ...cssLoaders.nodeModules],
    },
    {
      test: /\.less$/,
      include: includeTest.bind(null, paths.appNodeModules),
  ...
  if (config.cssModulesExclude && config.cssModulesExclude.length) {
    ...
    rules = [
      ...rules,
      {
        test: /\.css$/,
        include,
        use: ['style', ...cssLoaders.noCSSModules],
      },
      {
        test: /\.less$/,
        include,
  ...
  if (env === 'production') rules.forEach(rule => rule.use.unshift(MiniCssExtractPlugin.loader));
  return rules;
}
